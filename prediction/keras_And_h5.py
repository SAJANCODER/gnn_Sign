...
# Prioritize .keras as it's the recommended format.
MODEL_PATH_KERAS = "best_hybrid_model.keras"
MODEL_PATH_H5 = "hand_gesture_model_012_only.h5"  # Fallback if .keras is not found
...
def load_trained_model(model_path_keras, model_path_h5):
    """
    Loads the trained Keras model, preferring .keras over .h5.
    ...
    if os.path.exists(model_path_keras):
        ...
        try:
            model = tf.keras.models.load_model(model_path_keras, custom_objects=custom_objects)
    ...
    if model is None and os.path.exists(model_path_h5):
        print(f"Attempting to load model from {model_path_h5} (fallback)...")
        try:
    ...
    if model is None:
        raise FileNotFoundError(
            "No trained model found at specified paths (.keras or .h5). Please ensure training was successful.")
...
def main():
    ...
    try:
        model = load_trained_model(MODEL_PATH_KERAS, MODEL_PATH_H5)
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            # For this model, if no hand, landmarks will be zeros, and it might predict a class
            # or be unsure. Clearing history helps.
